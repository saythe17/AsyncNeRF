...
class DefaultDataset():
    def __init__(self, hparams, split='train'):
        ...
        if split == 'test':
            ...
            self.poses_matrices = [torch.tensor(np.loadtxt(x), dtype=torch.float64).reshape(3, 4) for x in self.src_files if x.suffix == '.txt']
            self.poses_matrices = torch.cat(self.poses_matrices, dim=0).reshape(-1, 3, 4)
