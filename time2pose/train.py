...
class Runner:
    ...
    def setup_environment(self):
        ...
        self.exp_name = str(
            max(
                [
                    int(name.name)
                    for name in self.exp_folder.iterdir()
                    if str.isdigit(name.name)
                ]
                + [0]
            )
            + 1
    ...
    def run(self):
        ...
        for epoch in bar:
            ...
            key_alias = {
                "loss": "loss",
                "rotation loss": "l_rot",
                "translation loss": "l_trans",
                "velocity loss": "l_velocity",
                "error_translation_mean": "val/err_trans",
    ...
    def _training_step(self, epoch):
        ...
        for batch in self.train_dataset:
            ...
            if self.hparams.use_manifold:
                ...
                out_rmat = rpmg.RPMG.apply(
                    predicted_rot,
                    tau,
                    self.hparams.manifold_lambda,
                    gt_rmat,
                    self.hparams.rotation_weight,
                )
                mse_ori = F.mse_loss(out_rmat, gt_rmat, reduction="mean")
                loss = (
                    self.hparams.translation_weight * trans_loss_raw
                    + mse_ori
                    + velocity_loss * self.hparams.velocity_weight
                )
                metrics["manifold loss"] = float(mse_ori)
                loss.backward()
            else:
                loss = (
                    trans_loss + rot_loss + velocity_loss * self.hparams.velocity_weight
                )
                loss.backward()
    ...
    def _run_validation(self):
        ...
        error_trans_axes = (
            pred_trans - gt_SE3.translation()
        ).abs().cpu().numpy() * self.hparams.pose_scale_factor
        error_trans = np.linalg.norm(error_trans_axes, axis=-1)
        theta_rot = (
            (
                torch.acos(
                    torch.sum(pred_rot * gt_SE3.rotation().tensor(), dim=-1)
                    .abs()
                    .clamp(-1, 1)
                )
                * 360
    ...
    def _save_checkpoint(self, epoch):
        ...
        with open(checkpoint_dir / f"{epoch}_loss_params.txt", mode="w") as f:
            f.write(
                f"s_x = {float(self.adaptive_loss_fn.s_x.data)}, s_q = {float(self.adaptive_loss_fn.s_q.data)}"
            )
