...
class Runner:
    ...
    def setup_environment(self):
        ...
        self.exp_name = str(
            max([int(name.name) for name in self.exp_folder.iterdir() if str.isdigit(name.name)] + [0]) + 1)
    ...
    def setup_network(self):
        self.network = SirenNet(
            ...
            dim_out=7,  # output dimension, ex. rgb value
            num_layers=3,  # number of layers
            final_activation=nn.Identity(),  # activation of final layer (nn.Identity() for direct output)
            w0_initial=20.,   # different signals may require different omega_0 in the first layer
            dropout=0.6
        ).to(self.device)
    ...
    def run(self):
        ...
        for epoch in bar:
            ...
            key_alias = {'loss': 'loss', 'rotation loss': 'l_rot', 'translation loss': 'l_trans', \
                         'velocity loss': 'l_velocity', 'error_translation_mean': "val/err_trans",
                         'theta_mean': 'val/theta'}
    ...
    def _training_step(self, epoch):
        ...
        for batch in self.train_dataset:
            ...
            predict = self.network(timestamps)
            predicted_trans = predict[:, :3]
            predicted_rot = predict[:, 3:]
            metrics = {}

            self.optimizer.zero_grad(set_to_none=True)
            gt_se3 = batch['SE3'].reshape(-1, 7).to(self.device)
            trans_loss, rot_loss, trans_loss_raw, rot_loss_raw = self.adaptive_loss_fn(
                torch.cat([predicted_trans, predicted_rot], dim=-1), gt_se3)
            velocity_loss = 0
            if self.hparams.use_manifold:
                ...
                out_rmat = rpmg.RPMG.apply(predicted_rot, tau, self.hparams.manifold_lambda, gt_rmat,
                                           self.hparams.rotation_weight)
    ...
    def _run_validation(self):
        ...
        for batch in self.val_dataset:
            ...
            predict = self.network(timestamp)
            predicted_trans = predict[:, :3]
            predicted_rot = predict[:, 3:]
        ...
        # if self.epoch % self.hparams.val_interval == 0:
        # pred_SE3 = torch.concatenate((pred_trans, pred_rot), dim=1)
        # data = pp.SE3(pred_SE3).matrix()[:, :3, :].reshape([-1, 12]).cpu().numpy()
        # np.savetxt('./siren.txt', data)
        ...
        theta_rot = (torch.acos(
            torch.sum(pred_rot * gt_SE3.rotation().tensor(), dim=-1).abs().clamp(-1, 1)) * 360 / math.pi).cpu().numpy()
    ...
    def _eval(self, epoch):
        ...
        for batch in self.test_dataset:
            ...
            predicted_trans = predict[:, :3]
            predicted_rot = predict[:, 3:]
            out_mat = pp.SE3(torch.cat([predict[:, :3], predict[:, 3:]], dim=-1)).matrix()
            for i in range(len(timestamps)):
                ts = int(float(timestamps[i]) + self.hparams.start_timestamp)
